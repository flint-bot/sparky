...
const People = (Spark) => {
  const people = {
     ...
     *   .catch(err => console.error(err));
     */
    personAdd: (person, testTrackingId = '') => {
      // if person object is valid
      if (person && typeof person === 'object' && validator.isPerson(person)) {
        // add person
        return Spark.request('post', 'people', person, testTrackingId);
      }
     ...
     *   .catch(err => console.error(err));
     */
    personUpdate: (person, testTrackingId = '') => {
      // if person object is valid
      if (person && typeof person === 'object' && validator.isPerson(person)) {
        // update the person
        return Spark.request('put', 'people', person.id, person, testTrackingId);
      }
      return when.reject(new Error('invalid or missing person object'));
    },
     ...
     *   .catch(err => console.error(err));
     */
    personRemove: (personId, testTrackingId = '') => {
      if (typeof personId !== 'string') {
        return when.reject(new Error('invalid arguments'));
      }
      return Spark.request('delete', 'people', personId, testTrackingId);
    },
