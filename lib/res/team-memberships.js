  ...
  const teamMemberships = {
    ...
    teamMembershipAdd: (...args) => {
      ...
      if (membershipObj && validator.isTeamMembershipSearch(membershipObj)) {
        return Spark.request('post', 'team/memberships', membershipObj, testTrackingId);
      }
     ...
     *   .catch(err => console.error(err));
     */
    teamMembershipUpdate: (teamMembership, testTrackingId = '') => {
      ...
      if (teamMembership && typeof teamMembership === 'object' && validator.isTeamMembership(teamMembership)) {
        ...
        return Spark.request('put', 'people', teamMembership.id, teamMembership, testTrackingId);
      }
     ...
     *   .catch(err => console.error(err));
     */
    teamMembershipRemove: (membershipId, testTrackingId = '') => {
      if (typeof membershipId !== 'string') {
        return when.reject(new Error('invalid arguments'));
      }
      return Spark.request('delete', 'team/memberships', membershipId, testTrackingId);
    },
  };
