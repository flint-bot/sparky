...
const Memberships = (Spark) => {
  const memberships = {
    ...
    membershipAdd: (...args) => {
      ...
      const personEmail = args.length > 0 && typeof args[0] === 'string' && roomId === false ? args.shift() : false;
      const isModerator = args.length > 0 && typeof args[0] === 'boolean' ? args.shift() : false;
      const testTrackingId = args.length > 0 && typeof args[0] === 'string' ? args.shift() : '';
      if (membershipObj && validator.isMembershipSearch(membershipObj)) {
        return Spark.request('post', 'memberships', membershipObj, testTrackingId);
      }
     ...
     * @param {string} testTrackingId optional testTrackingId for requests
     * @returns {Promise.<Membership>} Spark Membership object
     *
     * @example
     * spark.membershipGet('Tm90aGluZyB0byBzZWUgaGVy')
     ...
     *   .catch(err => console.error(err));
     */
    membershipUpdate: (membership, testTrackingId = '') => {
      // if membership object is valid
      if (membership && typeof membership === 'object' && validator.isMembership(membership)) {
        // update the membership
        return Spark.request('put', 'memberships', membership.id, membership, testTrackingId);
      }
     ...
     *   .catch(err => console.error(err));
     */
    membershipRemove: (membershipId, testTrackingId = '') => {
      if (typeof membershipId !== 'string') {
        return when.reject(new Error('invalid arguments'));
      }
      return Spark.request('delete', 'memberships', membershipId, testTrackingId);
    },
  };
