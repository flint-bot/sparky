...
function Sparky(config) {
  ...
  self.queue.on('dropped', function(dropped) {
    // re-emit event to sparky dropped
  ...
  self.on('dropped', function(request) {
...
Sparky.prototype.call = function(method, resource, id, query, callback) {
  ...
  function requestCallback(err, response, body) {
    ...
    if(err) {
      ...
      err = self.errorHandler('submiting spark api request');
      ...
      // if response without body
      else if(response.ok && response.headers && !response.body) {
        // generate error for processing response
        err = self.errorHandler('no response body');
