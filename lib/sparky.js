...
function Sparky(config) {
  ...
  self._requeueMinTime = self._minTime * 10;

  // max number of attempt to make for failed response
  self._requeueMaxRetry = self.config.requeueMaxRetry || 3;

  // calculate queue sizes for limiter
  self._queueSize = Math.floor(self._maxItems * 1000 / self._minTime);
  self._requeueSize = Math.floor(self._queueSize * 4);
  ...
  self.on('response', function(response) {
    if(self.DEBUG && response && response.headers && response.headers.trackingid) {
      ...
      debug('response: %s', trackingid || '<empty>'); 
    }
  });
  self.on('retry', function(response) {
    if(self.DEBUG && response && response.headers && response.headers.trackingid) {
      var trackingid = response.headers.trackingid;
      debug('retry: %s', trackingid || '<empty>'); 
    }
  });
  ...
  // alias functions
  self.message.sendRoom = self.message.send.room;
  self.message.sendPerson = self.message.send.person;
  self.membership.setModerator = self.membership.set.moderator
  self.membership.clearModerator = self.membership.clear.moderator;
  self.webhook.room = self.webhook.add.messages.created.room;
...
Sparky.prototype.call = function(method, resource, id, query, callback) {
  ...
  // emit request event
  self.emit('request', url, requestOptions);
  ...
  function requestCallback(err, response, body) {
    if(err) {
      ...
      err = errorHandler('recieved error processing spark api response');
      ...
      // catch 429 received from API overload
      ...
      // catch file response from contents api
        ...
        && resource === 'contents') 
      {
        callback ? callback(null, response) : null;
      }
      ...
      // if normal response
        ...
        if(response.body.items && response.body.items.length > 0) {
          callback ? callback(null, response.body.items) : null;
        }
        
        // if items array not found, single object located in body
        else if(!response.body.items && response.body.id) {
          callback ? callback(null, [ response.body ]) : null;
        }
        ...
        err = errorHandler('recieved %s processing spark api response', response.statusCode);
  ...
  // retry request
  function retryRequest(response) {
    // increment error counter
    errorCount++;
    
    // if error count exceeded
    if(errorCount > self._requeueMaxRetry) {
      var err = errorHandler('failed after %s retries', self._requeueMaxRetry);
      
      callback ? callback(err, null) : null;
    } else {
      // emit error for retry count
      errorHandler('retry #%s for request that generated %s response', errorCount, response.statusCode || 0);
  ...
  function request(cb){
    // send request
    req(requestOptions, requestCallback).on('end', cb);
  }
