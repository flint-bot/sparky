 ...
 *   });
 */
Spark.prototype.organizationsGet = function(maxResults) {
  return this.request('get', 'organizations', { max: this.maxPageItems }, maxResults)
    .then(res => this.toArray(res));
};
 ...
 *   });
 */
Spark.prototype.organizationGet = function(orgId) {
 ...
 *   });
 */
Spark.prototype.licensesGet = function(maxResults) {
  return this.request('get', 'licenses', { max: this.maxPageItems }, maxResults)
    .then(res => this.toArray(res));
};
 ...
 *   });
 */
Spark.prototype.licenseGet = function(licenseId) {
   ...
   * @property {int} totalUnits - Total units of license available
   * @property {int} consumedUnits - Number of license units consumed
   */
  return this.request('get', 'licenses', licenseId)
    .then(res => this.toObject(res));
};
 ...
 *   });
 */
Spark.prototype.rolesGet = function(maxResults) {
  return this.request('get', 'roles', { max: this.maxPageItems }, maxResults)
    .then(res => this.toArray(res));
};
 ...
 *   });
 */
Spark.prototype.roleGet = function(roleId) {
 ...
 *
 * @function
 * @param {String} [emails] - Email addresses for person
 * @param {String} displayName - Display name for person
 * @param {String} firstName - First name of the person being added
 * @param {String} lastName - Last name of the person being added
 * @param {String} avatar - Url to an image to upload as the avatar of the person being added
 * @param {String} orgId - Organization ID to add the person to
 * @param {String} [roles] - Roles to assign to the person
 * @param {String} [licenses] - Licenses to assign to the person
 * @returns {Promise.<Person>}
 *
 * @example
 ...
 *   });
 */
Spark.prototype.personAdd = function(emails, displayName, firstName, lastName, avatar, orgId, roles, licenses) {
  var config = {};

  // verify email address is defined
  if(emails && emails[0] && typeof emails[0] === 'string') {
    config.emails = emails;
  } else {
    return when.reject(new Error('email address not specified'));
  }

  // verify displayname OR firstname/lastName
  if(typeof displayName !== 'string' && (typeof firstName !== 'string' || typeof lastName !== 'string' )) {
      return when.reject(new Error('either displayName or firstName/lastName is required'));
  } else {
      if (typeof displayName === 'string') config.displayName = displayName;
      if (typeof firstName === 'string') config.firstName = firstName;
      if (typeof lastName === 'string') config.lastName = lastName;
  }
  ...
  // add optional roles
  if(roles && roles[0] && typeof roles[0] === 'string') {
    config.roles = roles;
  }

  // add optional licenses
  if(licenses && licenses[0] && typeof licenses[0] === 'string') {
    config.licenses = licenses;
  }

  // add person
  return this.request('post', 'people', config)
    .then(res => this.toObject(res));
};
 ...
 *   });
 */
Spark.prototype.personUpdate = function(personId, emails, displayName, firstName, lastName, avatar, orgId, roles, licenses) {
  ...
  // update the person
  return this.request('put', 'people', personId, config)
    .then(res => this.toObject(res));
}
 ...
 *
 * @function
 * @param {String} personId - Spark Team ID
 * @returns {Promise}
 *
 * @example
 ...
 *   });
 */
Spark.prototype.personRemove = function(personId) {
