...
const Messages = (Spark) => {
  const messages = {
     ...
     *   .catch(err => console.error(err));
     */
    messageSend: (message, file, testTrackingId = '') => {
      ...
      if (isMessage && !file) {
        return Spark.request('post', 'messages', message, testTrackingId);
      }
     ...
     *   .catch(err => console.error(err));
     */
    messageRemove: (messageId, testTrackingId = '') => {
      if (typeof messageId !== 'string') {
        return when.reject(new Error('invalid arguments'));
      }
      return Spark.request('delete', 'messages', messageId, testTrackingId);
    },
  };
