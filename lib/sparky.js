...
function Sparky(config) {
  ...
  self._queueDepthTime = self.config.queueDepthTime || 10000;
  self._requeueDepthTime = self.config.requeueDepthTime || Math.floor(self._queueDepthTime * self._requeueMinTime / self._minTime);
  ...
  // queue for API calls that overflow
  self.overflow = new Bottleneck(self._maxConcurrent, Math.floor(self._minTime / 2), self._queueSize * 2);
  ...
  self.on('response', function(response) {
    if(self.DEBUG && response && response.headers) {
      debug('received response %s in %s(ms) with tracking id: %s', response.statusCode, response.elapsedTime, response.headers.trackingid || '<empty>'); 
    } 
    else if(response && response.headers && !response.ok) {
      debug('received response %s in %s(ms) with tracking id: %s', response.statusCode, response.elapsedTime, response.headers.trackingid || '<empty>');
    }
  });
  ...
  self.queue.on('dropped', function(request) {
    self.queueRequest(self.overflow, request);
  });

  // emit drop events from queue
  self.overflow.on('dropped', function(request) {
...
// queue request
Sparky.prototype.queueRequest = function(queue, request) {
  queue.submit(request, null);
}
...
Sparky.prototype.call = function(method, resource, id, query, callback) {
  ...
  function request(end){
    req(requestOptions, requestCallback).on('end', end);
  }
  ...
  function requestCallback(err, response, body) {
    if(err) {
      ...
      err = self.errorHandler('recieved error processing spark api response');
      ...
      // if normal response
        ...
        err = self.errorHandler('recieved %s processing spark api response', response.statusCode);
  ...
  self.queueRequest(self.queue, request);
};
