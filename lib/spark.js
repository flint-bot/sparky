...
class Spark extends EventEmitter {
  ...
  request(...args) {
    ...
    const reResource = /^(contents|licenses|memberships|messages|organizations|people|roles|rooms|team\/memberships|teams|webhooks)$/i;
    ...
    if (method && resource) {
      ...
      const processResponse = (res, opts) => {
        ...
        const process429 = (_res, _opts) => {
          ...
          if (retryAfter > 0) {
            console.log(`Spark API rate limit exceeded and response will be delayed for ${retryAfter}s before being reattempted`);
          }
        ...
        const processError = (_res, _opts) => {
          const err = new Error(`recieved error ${_res.statucCode} for a ${_opts.method.toUpperCase()} request to ${_opts.url}`);
          console.error(err);
    ...
    const err = new Error('missing required arguemnts');
    console.error(err);
    return when.reject(err);
  }
