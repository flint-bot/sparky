  ...
  const webhooks = {
    ...
    webhookListen: () => {
      ...
      const webhookHandler = (req, res, next) => {
        ...
        const processBody = (bodyObj, testTrackingId = '') => {
          ...
          if (resource && event && data) {
            ...
            if (resource === 'messages' && event === 'created') {
              Spark.messageGet(data.id)
                .then((messageFull) => {
                  const messageWithTracking = messageFull;
                  messageWithTracking.testTrackingId = testTrackingId;
                  Spark.emit(resource, event, messageWithTracking, bodyObj);
                  Spark.emit(`${resource}-${event}`, messageWithTracking, bodyObj);
                })
                .catch(err => console.error(err));
            } else {
        ...
        if (req && typeof req === 'object' && _.has(req, 'headers') && _.has(req, Spark.webhookReqNamespace)) {
          ...
          // if this came from the emulator, lets see if there is a test-tracking-id
          const testTrackingId = _.has(headers, 'x-test-tracking-id') ? headers['x-test-tracking-id'] : '';
          ...
          if (sig && Spark.webhookSecret) {
            when(body)
              ...
              .then(bodyObj => when(processBody(bodyObj, testTrackingId)))
              .catch(err => console.error(err));
          } else if (xor(sig, Spark.webhookSecret)) {
            console.error(new Error('webhookListen ignored callback due to inconsistent webhook signature or secret'));
          } else {
            when(body)
              .then(bodyObj => when(processBody(bodyObj, testTrackingId)))
              .catch(err => console.error(err));
          ...
          console.error(new Error('invalid webhook received'));
        }
      };
