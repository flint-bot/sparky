...
function Spark(options) {
  ...
  // validate this.token exists
  if(typeof this.token !== 'string') throw new Error('Spark token not specified');
...
Spark.prototype.request = function(method, resource, id, query, maxResults) {
    ...
    return when(this.queue.schedule(request, options))
      .then(response => {
        ...
        if(_.includes(this.requeueCodes, response.statusCode)) {
          return requestRequeue(options).delay(this.requeueMinTime);
        } else {
  ...
  var requestPaginate = (items, link) => {
    ...
    // limit pagination depth if specified
