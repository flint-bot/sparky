...
const Webhooks = (Spark) => {
  const webhooks = {
    /**
     * @description Return all Spark Webhooks that the authenticated account is
     ...
     */
    webhooksSearch: (...args) => {
      const webhookSearch = args.length > 0 && typeof args[0] === 'object' ? args.shift() : {};
      const maxResults = args.length > 0 && typeof args[0] === 'number' ? args.shift() : 10;
      return Spark.request('get', 'webhooks', { max: Spark.maxPageItems }, 0)
        .then(webhooksAll => when(_.filter(webhooksAll, webhookSearch).slice(0, maxResults)));
    },
    ...
    webhookListen: () => {
      ...
      const webhookHandler = (req, res, next) => {
        ...
        if (req && typeof req === 'object' && _.has(req, 'headers') && _.has(req, Spark.webhookReqNamespace)) {
          ...
          if (xor(sig, Spark.webhookSecret)) {
