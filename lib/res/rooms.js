...
const Rooms = (Spark) => {
  const rooms = {
     ...
     *   .catch(err => console.error(err));
     */
    roomAdd: (title, teamId, testTrackingId = '') => {
      const room = { title };
      if (typeof teamId === 'string') {
        room.teamId = teamId;
      }
      return Spark.request('post', 'rooms', room, testTrackingId);
    },
     ...
     *   .catch(err => console.error(err));
     */
    roomUpdate: (room, testTrackingId = '') => {
      if (room && typeof room === 'object' && validator.isRoom(room)) {
        return Spark.request('put', 'rooms', room.id, room, testTrackingId);
      }
     ...
     *   .catch(err => console.error(err));
     */
    roomRemove: (roomId, testTrackingId = '') => {
      if (typeof roomId !== 'string') {
        return when.reject(new Error('invalid arguments'));
      }
      return Spark.request('delete', 'rooms', roomId, testTrackingId);
    },
  };
