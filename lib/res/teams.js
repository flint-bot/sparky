...
const Teams = (Spark) => {
  const teams = {
     ...
     *   .catch(err => console.error(err));
     */
    teamAdd: (name, testTrackingId = '') => {
      if (typeof name !== 'string') {
        return when.reject(new Error('invalid arguments'));
      }
      return Spark.request('post', 'teams', { name }, testTrackingId);
    },
     ...
     *   .catch(err => console.error(err));
     */
    teamUpdate: (team, testTrackingId = '') => {
      // if team object is valid
      if (team && typeof team === 'object' && validator.isTeam(team)) {
        // update the team
        return Spark.request('put', 'people', team.id, team, testTrackingId);
      }
     ...
     *   .catch(err => console.error(err));
     */
    teamRemove: (teamId, testTrackingId = '') => {
      if (typeof teamId !== 'string') {
        return when.reject(new Error('invalid arguments'));
      }
      return Spark.request('delete', 'teams', teamId, testTrackingId);
    },
  };
