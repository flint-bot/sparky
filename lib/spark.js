...
function Spark(options) {
   ...
   * @property {string} [maxPageItems=50] - Max results that the paginator uses.
   * @property {string} [maxConcurrent=3] - Max concurrent sessions to the Spark API
   * @property {string} [minTime=600] - Min time between consecutive request starts.
   * @property {string} [requeueMinTime=minTime*10] - Min time between consecutive request starts of requests that have been re-queued.
   * @property {string} [requeueMaxRetry=3] - Msx number of atteempts to make for failed request.
   * @property {string} [requeueCodes=[429,500,503]] - Array of http result codes that should be retried.
   * @property {string} [requestTimeout=20000] - Timeout for an individual request recieving a response.
   * @property {string} [queueSize=10000] - Size of the buffer that holds outbound requests.
   * @property {string} [requeueSize=10000] - Size of the buffer that holds outbound re-queue requests.
   */
...
Spark.prototype.webhookAdd = function(resource, evt, name, roomId) {
  ...
  resource = args.shift();
  evt = args.shift();
  roomId = args.length > 0 ? args.pop() : null;
  name = args.length > 0 ? args.shift() : 'mywebhook';
